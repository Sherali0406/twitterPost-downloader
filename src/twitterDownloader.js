      ...
      if (tweetInfo.includes?.media) {
        for (const [index, media] of tweetInfo.includes.media.entries()) {
          try {
            ...
            // Photo uchun URL sifatini ta'minlash
            if (media.type === "photo") {
              mediaUrl = media.url.includes("name=")
                ? media.url.replace(/name=\w+$/, "name=orig")
                : media.url; // Agar `name=` qismi bo'lmasa, asl URL-dan foydalanamiz.
            }
            // Video uchun eng yuqori bitrateni tanlash
            ...
            // Animatsiyalangan GIF uchun URL
            ...
            if (media.type === "photo") {
              const { width: imgWidth, height: imgHeight } = await this.getImageDimensions(mediaPath);
              width = imgWidth;
              height = imgHeight;
            ...
            if (!fs.existsSync(mediaPath)) {
              throw new Error(`Media download failed: ${mediaPath} does not exist.`);
            }
