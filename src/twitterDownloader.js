...
class TwitterDownloader {
  ...
  async downloadTweetMedia(twitterUrl, title = "", hashtags = []) {
    try {
        ...
        for (const [index, media] of tweetInfo.includes.media.entries()) {
          try {
            ...
            if (media.type === "video" && media.variants) {
              ...
              mediaUrl = highestBitrateVariant?.url;
            } else {
            ...
            if (mediaUrl) {
              ...
              if (media.type === "video" && !fs.existsSync(mediaPath)) {
                throw new Error(
                  `Video download failed: ${mediaPath} does not exist.`
                );
              }
      ...
      const tweetData = {
        ...
        hashtags:
          hashtags.length > 0
            ? hashtags
            : tweetInfo.data.entities?.hashtags?.map(
                (hashtag) => hashtag.tag
              ) || [],
        profileImagePath,
        mediaPaths,
