...
class TwitterDownloader {
  constructor(tokens) {
    if (!Array.isArray(tokens) || tokens.length === 0) {
      throw new Error(
        "At least one Bearer Token is required to initialize TwitterDownloader."
      );
    }
    ...
    try {
      ...
      if (tweetInfo.includes?.media) {
        ...
        for (const [index, media] of tweetInfo.includes.media.entries()) {
          try {
            console.log(
              `Processing media ${index + 1} with type: ${media.type}`
            );
            ...
            if (media.type === "photo") {
              if (isFirstPhotoSaved) {
                console.log(
                  `Skipping photo ${
                    index + 1
                  } as the first photo is already saved.`
                );
                continue;
              }
              ...
              if (variants.length > 0) {
                ...
                console.warn(
                  `No video variants available for media: ${JSON.stringify(
                    media
                  )}`
                );
              }
            ...
            if (!mediaUrl) {
              console.warn(
                `Media ${index + 1} URL not found. Media type: ${media.type}`
              );
              continue;
            }
            ...
            console.log(
              `Downloading media from URL: ${mediaUrl} to path: ${mediaPath}`
            );
            ...
            console.warn(
              `Error downloading media ${index + 1}:`,
              mediaError.message
            );
          }
        }
